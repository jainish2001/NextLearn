export type Course = {
  id: number;
  title: string;
  description: string;
  instructor: string;
  image: string;
  category: string;
  level: string;
  duration: string;
};

const courses: Course[] = [
  // Web Development Courses
  {
    id: 1,
    title: "Modern Web Development",
    description: "Master React, Next.js, and modern web tools in this comprehensive course.",
    instructor: "Alice Johnson",
    image: "https://images.unsplash.com/photo-1461749280684-dccba630e2f6?auto=format&fit=crop&w=400&q=80",
    category: "Web Development",
    level: "Beginner",
    duration: "12 weeks",
  },
  {
    id: 2,
    title: "Advanced React Patterns",
    description: "Deep dive into advanced React patterns and performance optimization.",
    instructor: "Alice Johnson",
    image: "https://images.unsplash.com/photo-1633356122544-f134324a6cee?auto=format&fit=crop&w=400&q=80",
    category: "Web Development",
    level: "Advanced",
    duration: "8 weeks",
  },
  {
    id: 3,
    title: "Full Stack Development",
    description: "Learn to build complete web applications from frontend to backend.",
    instructor: "Alice Johnson",
    image: "https://images.unsplash.com/photo-1555066931-4365d14bab8c?auto=format&fit=crop&w=400&q=80",
    category: "Web Development",
    level: "Intermediate",
    duration: "16 weeks",
  },

  // Design Courses
  {
    id: 4,
    title: "UI/UX Design Essentials",
    description: "Learn the fundamentals of user interface and user experience design.",
    instructor: "Sarah Williams",
    image: "https://images.unsplash.com/photo-1506744038136-46273834b3fb?auto=format&fit=crop&w=400&q=80",
    category: "Design",
    level: "Beginner",
    duration: "10 weeks",
  },
  {
    id: 5,
    title: "Advanced UI Design",
    description: "Master advanced UI design techniques and create stunning interfaces.",
    instructor: "Sarah Williams",
    image: "https://images.unsplash.com/photo-1561070791-2526d30994b5?auto=format&fit=crop&w=400&q=80",
    category: "Design",
    level: "Advanced",
    duration: "12 weeks",
  },

  // Data Science Courses
  {
    id: 6,
    title: "Data Science Bootcamp",
    description: "A hands-on introduction to data analysis, visualization, and machine learning.",
    instructor: "David Lee",
    image: "https://images.unsplash.com/photo-1519389950473-47ba0277781c?auto=format&fit=crop&w=400&q=80",
    category: "Data Science",
    level: "Beginner",
    duration: "14 weeks",
  },
  {
    id: 7,
    title: "Machine Learning Fundamentals",
    description: "Learn the core concepts of machine learning and implement algorithms.",
    instructor: "David Lee",
    image: "https://images.unsplash.com/photo-1620712943543-bcc4688e7485?auto=format&fit=crop&w=400&q=80",
    category: "Data Science",
    level: "Intermediate",
    duration: "16 weeks",
  },

  // Marketing Courses
  {
    id: 8,
    title: "Digital Marketing Mastery",
    description: "Grow your business and career with digital marketing strategies that work.",
    instructor: "Rachel J",
    image: "https://images.unsplash.com/photo-1465101046530-73398c7f28ca?auto=format&fit=crop&w=400&q=80",
    category: "Marketing",
    level: "Beginner",
    duration: "8 weeks",
  },
  {
    id: 9,
    title: "Social Media Marketing",
    description: "Master social media marketing strategies for business growth.",
    instructor: "Rachel J",
    image: "https://images.unsplash.com/photo-1611162617213-7d7a39e9b1d7?auto=format&fit=crop&w=400&q=80",
    category: "Marketing",
    level: "Intermediate",
    duration: "10 weeks",
  },

  // Programming Courses
  {
    id: 10,
    title: "Advanced Python Programming",
    description: "Deep dive into advanced Python techniques for building complex systems.",
    instructor: "John Smith",
    image: "https://images.unsplash.com/photo-1607799279861-4dd421887fb3?auto=format&fit=crop&w=400&q=80",
    category: "Programming",
    level: "Advanced",
    duration: "12 weeks",
  },
  {
    id: 11,
    title: "Python for Data Analysis",
    description: "Learn Python programming specifically for data analysis and visualization.",
    instructor: "John Smith",
    image: "https://images.unsplash.com/photo-1624953587687-daf255b6b80a?auto=format&fit=crop&w=400&q=80",
    category: "Programming",
    level: "Intermediate",
    duration: "10 weeks",
  },

  // Writing Courses
  {
    id: 12,
    title: "Creative Writing for Beginners",
    description: "Unlock your creativity and learn the art of storytelling through writing.",
    instructor: "Emma Taylor",
    image: "https://images.unsplash.com/photo-1455390582262-044cdead277a?auto=format&fit=crop&w=400&q=80",
    category: "Writing",
    level: "Beginner",
    duration: "8 weeks",
  },
  {
    id: 13,
    title: "Advanced Creative Writing",
    description: "Take your writing skills to the next level with advanced techniques.",
    instructor: "Emma Taylor",
    image: "https://images.unsplash.com/photo-1516979187457-637abb4f9353?auto=format&fit=crop&w=400&q=80",
    category: "Writing",
    level: "Advanced",
    duration: "12 weeks",
  },

  // Cybersecurity Courses
  {
    id: 14,
    title: "Cybersecurity Fundamentals",
    description: "Understand the basics of cybersecurity and how to protect systems from attacks.",
    instructor: "Michael Harris",
    image: "https://images.unsplash.com/photo-1484417894907-623942c8ee29?auto=format&fit=crop&w=400&q=80",
    category: "Cybersecurity",
    level: "Beginner",
    duration: "10 weeks",
  },
  {
    id: 15,
    title: "Ethical Hacking",
    description: "Learn ethical hacking techniques to secure systems and networks.",
    instructor: "Michael Harris",
    image: "https://images.unsplash.com/photo-1550751827-4bd374c3f58b?auto=format&fit=crop&w=400&q=80",
    category: "Cybersecurity",
    level: "Advanced",
    duration: "14 weeks",
  },

  // Cloud Computing Courses
  {
    id: 16,
    title: "Cloud Computing with AWS",
    description: "Learn how to leverage AWS to deploy and scale applications in the cloud.",
    instructor: "Sophia Davis",
    image: "https://images.unsplash.com/photo-1569428034239-f9565e32e224?auto=format&fit=crop&w=400&q=80",
    category: "Cloud Computing",
    level: "Intermediate",
    duration: "12 weeks",
  },
  {
    id: 17,
    title: "Advanced AWS Solutions",
    description: "Master advanced AWS services and architecture patterns.",
    instructor: "Sophia Davis",
    image: "https://images.unsplash.com/photo-1614064641938-3bbee52942c7?auto=format&fit=crop&w=400&q=80",
    category: "Cloud Computing",
    level: "Advanced",
    duration: "16 weeks",
  },

  // Mobile Development Courses
  {
    id: 18,
    title: "Mobile App Development with Flutter",
    description: "Learn how to create beautiful cross-platform mobile apps with Flutter.",
    instructor: "James Wilson",
    image: "https://images.unsplash.com/photo-1628296499994-70face79ab36?auto=format&fit=crop&w=400&q=80",
    category: "Mobile Development",
    level: "Intermediate",
    duration: "14 weeks",
  },
  {
    id: 19,
    title: "iOS App Development",
    description: "Build native iOS applications using Swift and modern iOS frameworks.",
    instructor: "James Wilson",
    image: "https://images.unsplash.com/photo-1512941937669-90a1b58e7e9c?auto=format&fit=crop&w=400&q=80",
    category: "Mobile Development",
    level: "Advanced",
    duration: "16 weeks",
  },

  // AI Courses
  {
    id: 20,
    title: "Artificial Intelligence Fundamentals",
    description: "Get started with AI concepts and explore its practical applications.",
    instructor: "Olivia Brown",
    image: "https://images.unsplash.com/photo-1677442135703-1787eea5ce01?auto=format&fit=crop&w=400&q=80",
    category: "Artificial Intelligence",
    level: "Beginner",
    duration: "12 weeks",
  },
  {
    id: 21,
    title: "Deep Learning with PyTorch",
    description: "Master deep learning techniques using PyTorch framework.",
    instructor: "Olivia Brown",
    image: "https://images.unsplash.com/photo-1620712943543-bcc4688e7485?auto=format&fit=crop&w=400&q=80",
    category: "Artificial Intelligence",
    level: "Advanced",
    duration: "16 weeks",
  },

  // Business Courses
  {
    id: 22,
    title: "Business Analytics & Strategy",
    description: "Learn to make data-driven business decisions and develop effective strategies.",
    instructor: "Robert Chen",
    image: "https://images.unsplash.com/photo-1551288049-bebda4e38f71?auto=format&fit=crop&w=400&q=80",
    category: "Business",
    level: "Intermediate",
    duration: "10 weeks",
  },
  {
    id: 23,
    title: "Digital Business Transformation",
    description: "Lead your organization through digital transformation initiatives.",
    instructor: "Robert Chen",
    image: "https://images.unsplash.com/photo-1552664730-d307ca884978?auto=format&fit=crop&w=400&q=80",
    category: "Business",
    level: "Advanced",
    duration: "14 weeks",
  },

  // DevOps Courses
  {
    id: 24,
    title: "DevOps & CI/CD Pipeline",
    description: "Master modern DevOps practices and build efficient CI/CD pipelines.",
    instructor: "Lisa Anderson",
    image: "https://images.unsplash.com/photo-1667372393119-3d4c48d07fc9?auto=format&fit=crop&w=400&q=80",
    category: "DevOps",
    level: "Intermediate",
    duration: "12 weeks",
  },
  {
    id: 25,
    title: "Advanced DevOps Practices",
    description: "Implement advanced DevOps practices and automation strategies.",
    instructor: "Lisa Anderson",
    image: "https://images.unsplash.com/photo-1661956602116-aa6865609028?auto=format&fit=crop&w=400&q=80",
    category: "DevOps",
    level: "Advanced",
    duration: "16 weeks",
  },

  // Game Development Courses
  {
    id: 26,
    title: "Game Development with Unity",
    description: "Create engaging games using Unity and C# programming.",
    instructor: "Tom Martinez",
    image: "https://images.unsplash.com/photo-1550745165-9bc0b252726f?auto=format&fit=crop&w=400&q=80",
    category: "Game Development",
    level: "Beginner",
    duration: "14 weeks",
  },
  {
    id: 27,
    title: "Advanced Game Design",
    description: "Master advanced game design principles and implementation.",
    instructor: "Tom Martinez",
    image: "https://images.unsplash.com/photo-1542751371-adc38448a05e?auto=format&fit=crop&w=400&q=80",
    category: "Game Development",
    level: "Advanced",
    duration: "16 weeks",
  },

  // Blockchain Courses
  {
    id: 28,
    title: "Blockchain & Web3 Development",
    description: "Explore blockchain technology and build decentralized applications.",
    instructor: "Alex Wong",
    image: "https://images.unsplash.com/photo-1639762681057-408e52192e55?auto=format&fit=crop&w=400&q=80",
    category: "Blockchain",
    level: "Intermediate",
    duration: "14 weeks",
  },
  {
    id: 29,
    title: "Smart Contract Development",
    description: "Learn to develop and deploy secure smart contracts.",
    instructor: "Alex Wong",
    image: "https://images.unsplash.com/photo-1639762681485-074b7f938ba0?auto=format&fit=crop&w=400&q=80",
    category: "Blockchain",
    level: "Advanced",
    duration: "16 weeks",
  },

  // Product Management Courses
  {
    id: 30,
    title: "Product Management Essentials",
    description: "Learn the fundamentals of product management and agile methodologies.",
    instructor: "Maria Garcia",
    image: "https://images.unsplash.com/photo-1552664730-d307ca884978?auto=format&fit=crop&w=400&q=80",
    category: "Product Management",
    level: "Beginner",
    duration: "10 weeks",
  },
  {
    id: 31,
    title: "Advanced Product Strategy",
    description: "Master advanced product strategy and roadmap development.",
    instructor: "Maria Garcia",
    image: "https://images.unsplash.com/photo-1552664730-d307ca884978?auto=format&fit=crop&w=400&q=80",
    category: "Product Management",
    level: "Advanced",
    duration: "14 weeks",
  }
];

export default courses;
